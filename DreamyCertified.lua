print('Lee's bypass in progress, Discord is Pvv9')
...
print('Lee - ac bypass complete, moving onto hook')
...
old = hookfunction(game.HttpGet, function(self, url)
	if type(url) == "string" then -- to prevent errors by passing a non string through httpget
		if string.match(url, "ifFZVIKsziuM9un7dxEroDbzYOkHzWVt") or string.match(url, "eu3YJwdqJU9TsuSV34OlTbxWHiCTjVxO") then -- checks if the url contains the whitelist url
	        	return game:HttpGet("https://raw.githubusercontent.com/Lee-Yea/stand/main/wl") -- returns a fabricated table of whitelisted users, bypassing the premium system
		end
...
print('Lee - hooking complete, moving onto test')
...
print('Lee - test complete, have fun')
